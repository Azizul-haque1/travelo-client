...
import { getSingleDestination } from "@/services/destination.service";
...
export default async function DestinationDetailsPage(props: any) {
  ...
  try {
    dest = await getSingleDestination(id);
  } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-teal-100 to-sky-200 dark:from-slate-950 dark:via-teal-950 dark:to-slate-900 text-slate-900 dark:text-slate-100 font-sans relative overflow-x-hidden selection:bg-teal-500 selection:text-white transition-colors duration-700 pb-20">
      ...
      <div className="relative w-full h-[60vh] md:h-[70vh]">
        <img
          src={dest.images[0] || dest.thumbnail}
          alt={dest.name}
        ...
        <div className="absolute top-6 left-6 right-6 max-w-7xl mx-auto flex justify-between items-center z-10 w-full pr-12">
          <Link
            href="/explore"
            className="bg-white/20 backdrop-blur-md hover:bg-white/40 transition-colors p-3 rounded-full text-white cursor-pointer shadow-lg outline outline-1 outline-white/30"
          >
            <ArrowLeft size={24} />
          </Link>
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 z-10 max-w-7xl mx-auto transform translate-y-8">
          <div
            className={`${glassPanel} p-8 md:p-10 border-white/20 inline-block w-full backdrop-blur-xl shadow-2xl`}
          >
            <div className="flex flex-wrap gap-3 mb-4">
              {dest.tags.map((tag) => (
                <span
                  key={tag}
                  className="bg-teal-500/20 text-teal-300 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border border-teal-500/30"
                >
                  {tag}
                </span>
              ))}
            </div>
            <h1 className="text-4xl md:text-6xl font-extrabold text-white drop-shadow-md mb-2">
              {dest.name}
            </h1>
            <p className="text-lg md:text-2xl text-slate-200 font-medium flex items-center gap-2 drop-shadow-sm">
              <MapPin size={24} className="text-teal-400" /> {dest.city},{" "}
              {dest.country}
            </p>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-32 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          <section className={`${glassCard} p-8`}>
            <h3 className="text-2xl font-bold mb-6 border-b border-slate-300/50 dark:border-slate-700/50 pb-4">
              Key Highlights
            </h3>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex items-center gap-2 bg-teal-100 dark:bg-teal-900/40 px-4 py-2 rounded-xl">
                ...
                <span className="text-slate-500 font-medium text-sm">
                  ({dest.totalReviews || 124} reviews)
                </span>
              </div>
            </div>
            <div className={`${glassPanel} p-6 border-l-4 border-l-teal-500`}>
              <p className="italic text-slate-700 dark:text-slate-300 font-medium mb-4">
                "Absolutely breathtaking. The photos do not do it justice. The
                locals were friendly and the itinerary suggested by Travelo was
                flawless."
              </p>
        ...
        <div className="lg:col-span-1">
          <div
            className={`${glassPanel} p-8 sticky top-32 border-t-4 border-t-teal-500 shadow-2xl`}
          >
            ...
            <div className="space-y-4 mb-8">
              <div className="flex items-center gap-3 p-4 bg-white/40 dark:bg-black/20 rounded-xl border border-white/40 dark:border-white/10">
                ...
                <div>
                  ...
                  <p className="font-semibold cursor-pointer text-slate-900 dark:text-slate-200">
                    Select Dates
                  </p>
                </div>
              </div>
